      ...
      <div className="exam-content" style={{ zoom: `${zoom}%` }}>
        <div className="exam-question-column">
          <article className="exam-question-card">
            ...
            <div className="quiz-choices">
              {question.choices.map((choice, choiceIndex) => (
                <button
                  key={choice}
                  disabled={checked}
                  className={`${selected === choiceIndex ? "selected" : ""} ${checked && choiceIndex === question.answer ? "correct" : ""} ${checked && selected === choiceIndex && choiceIndex !== question.answer ? "wrong" : ""}`}
            ...
            {checked && (
              <div className={`quiz-result ${selected === question.answer ? "is-correct" : ""}`}>
                <strong>
                  {selected === question.answer
                    ? "정답입니다!"
                    : `정답은 ${choiceMarker(question.answer)}입니다.`}
                </strong>
            ...
            <div className="exam-question-actions quiz-question-actions">
              ...
              <button className="btn btn-next" disabled={!checked} onClick={next}>
                {last ? "학습 선택으로" : "다음 →"}
              </button>
            </div>
          </article>
