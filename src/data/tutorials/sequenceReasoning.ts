...
export const SEQUENCE_REASONING_TUTORIALS: Problem[] = [
  defineTutorial({
    ...
    strategy:
      "먼저 항 사이의 ‘차이’가 일정한지(등차) 보고, 아니면 ‘비’가 일정한지(등비) 확인하세요. 분수·소수라도 ‘몇을 더하거나 곱하면 다음 항이 되는지’만 찾으면 됩니다. 등비는 매 항에 같은 수를 곱해요.",
  ...
  defineTutorial({
    ...
    strategy:
      "익숙한 등차·등비가 안 보이면 ‘차이(계차)’나 ‘반복되는 연산’을 의심하세요. 항 사이 차이를 쭉 적어 −3, +0.5처럼 반복 패턴이 있는지 보고, 규칙을 찾으면 원하는 항까지 그대로 이어 계산합니다.",
    ...
    steps: [
      ...
      {
        narration: "즉 −3.0과 +0.5가 번갈아 반복되는 수열이에요. 7번째 항 1.0부터 규칙을 이어가면 됩니다.",
        highlight: [1],
      },
      {
        narration:
