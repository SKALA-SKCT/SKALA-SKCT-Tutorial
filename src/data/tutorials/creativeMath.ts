...
export const CREATIVE_MATH_TUTORIALS: Problem[] = [
  defineTutorial({
    ...
    strategy:
      "농도 문제는 ‘용질(소금이나 설탕)의 양은 섞기 전후가 같다’가 핵심이에요. 두 용액을 섞을 때는 공식 (C1−C3)×W1 = (C3−C2)×W2 (C3=최종 농도, W=양)를 쓰면 빠릅니다. 즉 ‘각 농도와 최종 농도의 차 × 양’이 서로 같습니다.",
    ...
    steps: [
      {
        narration:
          "주어진 값을 정리해요. A는 600g에 농도 30%(C1), B는 400g에 농도 C2(구할 값), 최종 농도는 26%(C3)입니다. ‘섞기 전후 설탕의 양이 같다’를 이용합니다.",
      },
      {
        narration:
  ...
  defineTutorial({
    ...
    strategy:
      "원가와 정가, 판매가 문제는 두 관계식으로 풉니다. ①판매가 = 정가 × (100−할인%)/100, ②이익 = 판매가 − 원가. ‘원가의 50% 이익’은 판매가 = 원가 × 1.5를 뜻해요. 구하는 값(주로 원가)을 미지수로 두고 식을 세우세요.",
  ...
  defineTutorial({
    ...
    steps: [
      ...
      {
        narration: "철수의 시간 = 2k = 1시간. 거리 = 속력 × 시간 = 60 × 1 = 60km. 정답은 ②입니다.",
        highlightChoices: [2],
        reveal: true,
      },
    ],
